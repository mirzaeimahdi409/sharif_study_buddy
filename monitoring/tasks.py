...
def ingest_message_to_kb(message: Message, channel_username: str):
    ...
    try:
        response = requests.post(settings.RAG_API_URL + '/documents/ingest-channel-message/', json=payload)
        response.raise_for_status()
        print(f"Successfully ingested message {message.id} from {channel_username}")
